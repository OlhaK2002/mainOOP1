...
if($text!="") {
    ...
    if($link->query("SELECT * FROM `comments` WHERE `author`='$author' and `text`='$text' and `parent_id`='$parent_id'"))
    {

        $sql2 = $link->query("SELECT * FROM `comments` WHERE `author`='$author' and `text`='$text' and `parent_id`='$parent_id'");
